use duper::visitor::DuperVisitor;
use js_sys::{Array, Object, Reflect, Symbol};
use wasm_bindgen::prelude::*;

#[derive(Clone)]
pub(crate) struct Visitor;

impl DuperVisitor for Visitor {
    type Value = Result<JsValue, JsError>;

    fn visit_object<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        object: &duper::DuperObject<'a>,
    ) -> Self::Value {
        let js_object = Object::new();

        for (key, value) in object.iter() {
            let value_result = value.accept(self)?;
            Reflect::set(&js_object, &JsValue::from(key.as_ref()), &value_result).map_err(|e| {
                JsError::new(&format!("Failed to set property {}: {:?}", key.as_ref(), e))
            })?;
        }

        if let Some(identifier) = identifier {
            let duper_symbol = Symbol::for_("duper.identifier");
            Reflect::set(
                &js_object,
                &duper_symbol,
                &JsValue::from_str(identifier.as_ref()),
            )
            .map_err(|e| JsError::new(&format!("Failed to set Duper identifier: {:?}", e)))?;
        }

        Ok(js_object.into())
    }

    fn visit_array<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        array: &duper::DuperArray<'a>,
    ) -> Self::Value {
        let js_array = Array::new();

        for (index, value) in array.iter().enumerate() {
            let value_result = value.accept(self)?;
            js_array.set(index as u32, value_result);
        }

        if let Some(identifier) = identifier {
            let duper_symbol = Symbol::for_("duper.identifier");
            Reflect::set(
                &js_array,
                &duper_symbol,
                &JsValue::from_str(identifier.as_ref()),
            )
            .map_err(|e| JsError::new(&format!("Failed to set Duper identifier: {:?}", e)))?;
        }

        Ok(js_array.into())
    }

    fn visit_tuple<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        tuple: &duper::DuperTuple<'a>,
    ) -> Self::Value {
        let js_array = Array::new();

        for (index, value) in tuple.iter().enumerate() {
            let value_result = value.accept(self)?;
            js_array.set(index as u32, value_result);
        }

        // Mark as tuple using symbol
        let tuple_symbol = Symbol::for_("duper.tuple");
        Reflect::set(&js_array, &tuple_symbol, &JsValue::from_bool(true))
            .map_err(|e| JsError::new(&format!("Failed to set tuple symbol: {:?}", e)))?;

        if let Some(identifier) = identifier {
            let duper_symbol = Symbol::for_("duper.identifier");
            Reflect::set(
                &js_array,
                &duper_symbol,
                &JsValue::from_str(identifier.as_ref()),
            )
            .map_err(|e| JsError::new(&format!("Failed to set Duper identifier: {:?}", e)))?;
        }

        Ok(js_array.into())
    }

    fn visit_string<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        string: &duper::DuperString<'a>,
    ) -> Self::Value {
        let value = JsValue::from_str(&string.clone().into_inner());
        attach_identifier(value, identifier)
    }

    fn visit_bytes<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        bytes: &duper::DuperBytes<'a>,
    ) -> Self::Value {
        let byte_slice = bytes.clone().into_inner();
        let uint8_array = js_sys::Uint8Array::from(byte_slice.as_ref());
        let value: JsValue = uint8_array.into();
        attach_identifier(value, identifier)
    }

    fn visit_integer<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        integer: i64,
    ) -> Self::Value {
        let value = JsValue::from_f64(integer as f64);
        attach_identifier(value, identifier)
    }

    fn visit_float<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        float: f64,
    ) -> Self::Value {
        let value = JsValue::from_f64(float);
        attach_identifier(value, identifier)
    }

    fn visit_boolean<'a>(
        &mut self,
        identifier: Option<&duper::DuperIdentifier<'a>>,
        boolean: bool,
    ) -> Self::Value {
        let value = JsValue::from_bool(boolean);
        attach_identifier(value, identifier)
    }

    fn visit_null<'a>(&mut self, identifier: Option<&duper::DuperIdentifier<'a>>) -> Self::Value {
        let value = JsValue::null();
        attach_identifier(value, identifier)
    }
}

fn attach_identifier(
    value: JsValue,
    identifier: Option<&duper::DuperIdentifier>,
) -> Result<JsValue, JsError> {
    if let Some(identifier) = identifier {
        let boxed = Object::new();
        let value_symbol = Symbol::for_("duper.value");
        let identifier_symbol = Symbol::for_("duper.identifier");

        Reflect::set(&boxed, &value_symbol, &value)
            .map_err(|e| JsError::new(&format!("Failed to set Duper value symbol: {:?}", e)))?;
        Reflect::set(
            &boxed,
            &identifier_symbol,
            &JsValue::from_str(identifier.as_ref()),
        )
        .map_err(|e| JsError::new(&format!("Failed to set Duper identifier: {:?}", e)))?;

        let to_json =
            js_sys::Function::new_with_args("", r#"return this[Symbol.for("duper.value")];"#);
        Reflect::set(&boxed, &JsValue::from("toJSON"), &to_json)
            .map_err(|e| JsError::new(&format!("Failed to set toJSON function: {:?}", e)))?;

        Ok(boxed.into())
    } else {
        Ok(value)
    }
}
