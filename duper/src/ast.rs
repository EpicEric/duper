...
/// A Duper Temporal value, which may be borrowed or owned.
#[derive(Debug, Clone, Hash, PartialEq, Eq)]
pub struct DuperTemporal<'a>(pub(crate) Cow<'a, str>);
...
impl<'a> DuperTemporal<'a> {
    /// Consume this Temporal value and return the underlying [`&str`].
    pub fn into_inner(self) -> Cow<'a, str> {
        self.0
    }
}
...
impl<'a> TryFrom<Cow<'a, str>> for DuperTemporal<'a> {
    ...
    fn try_from(value: Cow<'a, str>) -> Result<Self, Self::Error> {
        ...
        let invalid_char_pos = parser::temporal_inner()
            .parse(value.as_ref())
            .into_result()
            .err()
            .map(|errs| errs[0].span().start);
        match invalid_char_pos {
            Some(pos) => Err(DuperTemporalTryFromError::InvalidChar(value, pos)),
            None => Ok(Self(value)),
        }
    }
}
